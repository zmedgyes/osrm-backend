...
bool ExtractorCallbacks::wayFunction(_Way &w) {
    ...
    if ( w.speed > 0 ) { //Only true if the way is specified by the speed profile
        ...
        for(std::vector< NodeID >::size_type n = 0; n < w.path.size()-1; ++n) {
            externalMemory->allEdges.push_back(_Edge(w.path[n], w.path[n+1], w.type, w.direction, w.speed, w.impedance, w.nameID, w.roundabout, w.ignoreInGrid, w.isDurationSet, w.isAccessRestricted));
